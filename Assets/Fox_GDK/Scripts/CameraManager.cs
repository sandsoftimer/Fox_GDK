...
namespace Com.FunFox.Utility
{
    ...
    public class CameraManager : MonoBehaviour
    {
        ...
        public void ShakeDefaultCamera(float duration = 0.15f, float magnitude = 0.05f, bool isCameraLocal = false, Action action = null)
        {
            StartCoroutine(ShakeDefaultCam(Camera.main.transform, duration, magnitude, isCameraLocal, action));
        }

        public void ShakeParentOfCamera(float duration = 0.25f, float magnitude = 0.1f, bool isCameraLocal = false, Action action = null)
        {
            ...
            StartCoroutine(ShakeDefaultCam(camParent.transform, duration, magnitude, isCameraLocal, action));
        }

        IEnumerator ShakeDefaultCam(Transform cam, float duration, float magnitude, bool isCameraLocal = false, Action action = null)
        {
            if (isCameraLocal)
            {
        ...
        void CaptureEditorGameViewScreenshot()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Insert))
            {
                ...
                var theTime = DateTime.Now.ToString("hh_mm_ss");
                var theDate = DateTime.Now.ToString("MM_dd_yyyy");
