...
public class EasyObjectsFade : MonoBehaviour
{
    ...
    [Tooltip("If true, 2 separate lines are raycasted to detect meshes even when the sceneCam is inside")]
    [SerializeField] private bool detectFromInsideCollider = false;

    [Tooltip("Layers that can be faded, you can use it to hide player or other non-dithered shaders from this script, if you forget about exlcluding with row mask non-urpStandard shaders error will occur")]
