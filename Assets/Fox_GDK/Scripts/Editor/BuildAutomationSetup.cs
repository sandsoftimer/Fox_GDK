...
public class BuildAutomationSetup : EditorWindow
{
    ...
    private void SetupBuildAutomation()
    {
        ...
        CreateTestBuildWorkflow(workflowsFolder);
        CreateSubmissionBuildWorkflow(workflowsFolder);
    ...
    private void CreateTestBuildWorkflow(string workflowsFolder)
    {
        string testBuildPath = Path.Combine(workflowsFolder, "test-build.yml");
        string content = "name: Test Build - " + buildMessage + @"
  ...
  test-build:
    ...
    - uses: actions/upload-artifact@v4
      with:
        ...
        File.WriteAllText(testBuildPath, content);
    }

    private void CreateSubmissionBuildWorkflow(string workflowsFolder)
    {
        string submissionBuildPath = Path.Combine(workflowsFolder, "submission-build.yml");
        string content = "name: Submission Build - " + buildMessage + @"
  ...
  submission-build:
    ...
    - uses: actions/upload-artifact@v4
      with:
        ...
        File.WriteAllText(submissionBuildPath, content);
    }
}
#endif